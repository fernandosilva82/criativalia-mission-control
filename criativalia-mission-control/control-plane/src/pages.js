...
<head>
    ...
    <style>
        ...
        * { scrollbar-width: thin; scrollbar-color: #2a2a2a #0a0a0b; }
        ...
        .status-error { color: #c17767; }
        .execution-progress {
            background: linear-gradient(90deg, #c17767 var(--progress, 0%), #2a2a2a var(--progress, 0%));
        }
        .log-entry {
...
<body class="min-h-screen">
    ...
    <header class="border-b border-gray-800 bg-gray-900/50 backdrop-blur sticky top-0 z-50">
        <div class="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 id="agent-name" class="font-bold text-lg">Loading...</h1>
                    <p id="agent-role" class="text-xs text-gray-400">-</p>
                </div>
            </div>
            <div class="flex items-center gap-4">
                <span id="agent-status" class="px-3 py-1 rounded-full text-xs font-medium bg-gray-800">-</span>
                <a href="/" class="p-2 rounded-lg bg-gray-800 hover:bg-gray-700 transition">
                    <i class="fas fa-arrow-left"></i>
                </a>
            </div>
            ...
            <div class="space-y-6">
                ...
                <div class="card p-5">
                    <div class="flex items-center gap-4 mb-4">
                        <div id="agent-icon" class="w-16 h-16 rounded-xl bg-gray-800 flex items-center justify-center text-3xl">
                            🤖
                        </div>
                        <div>
                            <h2 id="agent-name-detail" class="font-bold">-</h2>
                            <p id="agent-role-detail" class="text-sm text-gray-400">-</p>
                        </div>
                    </div>
                    <p id="agent-description" class="text-sm text-gray-300 mb-4">-</p>
                    
                    <div class="space-y-3">
                        <div class="flex justify-between text-sm">
                            <span class="text-gray-400">Active Sessions</span>
                            <span id="stat-sessions" class="font-medium">0</span>
                        </div>
                        <div class="flex justify-between text-sm">
                            <span class="text-gray-400">Pending Tasks</span>
                            <span id="stat-tasks" class="font-medium">0</span>
                        </div>
                        <div class="flex justify-between text-sm">
                            <span class="text-gray-400">Total Executions</span>
                            <span id="stat-executions" class="font-medium">0</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="card p-5">
                    ...
                    <div id="agent-skills" class="flex flex-wrap gap-2">
                        <!-- Skills loaded here -->
                    </div>
                </div>
                ...
                <div class="card p-5">
                    ...
                    <pre id="agent-config" class="text-xs bg-gray-900 p-3 rounded-lg overflow-x-auto">{}</pre>
                </div>
            </div>

            <!-- Middle Column: Executor -->
            <div class="space-y-6">
                <!-- Execution Control -->
                <div class="card p-5">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <div class="flex items-center gap-2">
                            <span id="execution-status" class="text-xs text-gray-400">Ready</span>
                            <span id="status-dot" class="w-2 h-2 rounded-full bg-gray-500"></span>
                        </div>
                    </div>

                    <!-- Input Form -->
                    <div id="execution-form" class="space-y-4">
                        <div>
                            <label class="block text-xs text-gray-400 mb-1">Task / Objective</label>
                            <textarea id="execution-input" rows="3" class="w-full bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 text-sm focus:border-[#c17767] focus:outline-none" placeholder="Describe what you want the agent to do..."></textarea>
                        </div>
                        
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                <label class="block text-xs text-gray-400 mb-1">Workspace</label>
                                <input type="text" id="execution-workspace" class="w-full bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 text-sm" placeholder="auto-generated">
                            </div>
                            <div>
                                <label class="block text-xs text-gray-400 mb-1">Timeout (min)</label>
                                <input type="number" id="execution-timeout" value="30" min="1" max="120" class="w-full bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 text-sm">
                            </div>
                        </div>
                        
                        <div class="flex gap-2">
                            <button id="btn-execute" onclick="executeAgent()" class="flex-1 btn-primary py-2.5 rounded-lg text-sm font-medium flex items-center justify-center gap-2">
                                <i class="fas fa-play"></i>
                                Execute
                            </button>
                            <button id="btn-stop" onclick="stopExecution()" disabled class="btn-secondary px-4 py-2.5 rounded-lg text-sm font-medium flex items-center justify-center gap-2">
                                <i class="fas fa-stop"></i>
                            </button>
                        </div>
                    </div>
                    ...
                    <!-- Progress -->
                    <div id="execution-progress" class="hidden">
                        <div class="mb-3">
                            ...
                            <div class="h-2 bg-gray-800 rounded-full overflow-hidden">
                                <div id="progress-bar" class="h-full bg-[#c17767] transition-all duration-300" style="width: 0%"></div>
                            </div>
                        </div>
                        
                        <div id="execution-logs" class="bg-gray-900 rounded-lg p-3 h-48 overflow-y-auto font-mono text-xs space-y-1">
                            <!-- Logs appear here -->
                        </div>
                    </div>

                    <!-- Results -->
                    <div id="execution-results" class="hidden space-y-4">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-2">
                                <i id="result-icon" class="fas fa-check-circle text-green-400 text-xl"></i>
                                <div>
                                    <div id="result-status" class="font-medium">Completed</div>
                                    <div id="result-time" class="text-xs text-gray-400">Took 2m 34s</div>
                                </div>
                            </div>
                            <button onclick="resetExecution()" class="btn-secondary px-3 py-1.5 rounded-lg text-xs">
                                New Execution
                            </button>
                        </div>
                        
                        <div id="result-output" class="bg-gray-900 rounded-lg p-4 font-mono text-sm whitespace-pre-wrap">
                            <!-- Result output -->
                        </div>
                        
                        <div id="result-evidence" class="hidden">
                            <h4 class="text-xs font-medium text-gray-400 uppercase mb-2">Evidence Saved</h4>
                ...
                <div class="card">
                    ...
                    <div id="recent-executions" class="divide-y divide-gray-800 max-h-64 overflow-y-auto">
                        <!-- Executions loaded here -->
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-6">
                ...
                <div class="card">
                    <div class="p-4 border-b border-gray-800 flex items-center justify-between">
                        ...
                        <button onclick="createTask()" class="text-xs btn-primary px-2 py-1 rounded">
                            <i class="fas fa-plus mr-1"></i>New
                        </button>
                    </div>
                    <div id="agent-tasks" class="divide-y divide-gray-800 max-h-64 overflow-y-auto">
                        <div class="p-4 text-center text-gray-500 text-sm">No active tasks</div>
                    </div>
                </div>
                ...
                <div class="card">
                    ...
                    <div id="agent-sessions" class="divide-y divide-gray-800 max-h-64 overflow-y-auto">
                        <div class="p-4 text-center text-gray-500 text-sm">No sessions</div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        function renderAgentInfo() {
            ...
            document.getElementById('agent-name-detail').textContent = agent.name;
            document.getElementById('agent-role-detail').textContent = agent.role;
            document.getElementById('agent-description').textContent = agent.description || 'No description';
            document.getElementById('agent-icon').textContent = getAgentIcon(agent.role);
            
            // Status
            const statusEl = document.getElementById('agent-status');
            statusEl.textContent = agent.status;
            statusEl.className = \`px-3 py-1 rounded-full text-xs font-medium \${agent.status === 'running' ? 'bg-green-900/50 text-green-400' : 'bg-gray-800 text-gray-400'}\`;
            
            // Stats
            document.getElementById('stat-sessions').textContent = agent.sessions?.length || 0;
            document.getElementById('stat-tasks').textContent = agent.tasks?.filter(t => ['pending', 'in_progress'].includes(t.status)).length || 0;
            document.getElementById('stat-executions').textContent = agent.executions?.length || 0;
            
            // Skills
            const skillsContainer = document.getElementById('agent-skills');
            skillsContainer.innerHTML = agent.skills?.map(skill => 
                \`<span class="skill-tag px-2 py-1 rounded-lg text-xs">\${skill}</span>\`
            ).join('') || 'No skills';
        ...
        function renderTasks() {
            ...
            const activeTasks = agent.tasks?.filter(t => ['pending', 'in_progress', 'review'].includes(t.status)) || [];
            
            if (activeTasks.length === 0) {
                container.innerHTML = '<div class="p-4 text-center text-gray-500 text-sm">No active tasks</div>';
                return;
            }
            ...
            container.innerHTML = activeTasks.map(task => \`
                <div class="p-3 hover:bg-gray-800/50 transition">
                    <div class="flex items-start justify-between">
                        <div>
                            <div class="flex items-center gap-2">
                                <span class="text-[10px] font-medium px-1.5 py-0.5 rounded bg-gray-800">P\${task.priority}</span>
                                <h4 class="text-sm font-medium">\${task.title}</h4>
                            </div>
                            <p class="text-xs text-gray-500 mt-1">\${task.description?.substring(0, 50) || ''}...</p>
                        </div>
        ...
        function renderSessions() {
            ...
            if (agentSessions.length === 0) {
                container.innerHTML = '<div class="p-4 text-center text-gray-500 text-sm">No sessions</div>';
                return;
            }
            ...
            container.innerHTML = agentSessions.map(session => \`
                <div class="p-3 hover:bg-gray-800/50 transition">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-2">
                            <i class="fas fa-circle text-[8px] \${session.status === 'running' ? 'text-green-400' : 'text-gray-600'}"></i>
                            <span class="text-sm font-mono">\${session.id.slice(-8)}</span>
                        </div>
        ...
        function renderRecentExecutions() {
            ...
            if (executions.length === 0) {
                container.innerHTML = '<div class="p-4 text-center text-gray-500 text-sm">No executions yet</div>';
                return;
            }
            
            container.innerHTML = executions.slice(0, 10).map(exec => \`
                <div class="p-3 hover:bg-gray-800/50 transition">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-2">
                            <i class="fas \${exec.status === 'success' ? 'fa-check-circle text-green-400' : exec.status === 'failed' ? 'fa-times-circle text-red-400' : 'fa-spinner fa-spin text-blue-400'}"></i>
                            <span class="text-sm truncate max-w-[150px]">\${exec.input?.substring(0, 30) || 'Execution'}...</span>
                        </div>
        ...
        async function executeAgent() {
            const input = document.getElementById('execution-input').value.trim();
            const workspace = document.getElementById('execution-workspace').value.trim();
            ...
            // UI updates
            document.getElementById('execution-form').classList.add('hidden');
            ...
            document.getElementById('status-dot').className = 'w-2 h-2 rounded-full bg-green-400 animate-pulse';
            ...
            try {
                const res = await fetch(\`/api/agents/\${agentId}/execute\`, {
                    ...
                    body: JSON.stringify({
                        input,
                        workspace: workspace || \`workspaces/\${agentId}_\${Date.now()}\`,
                ...
                // Start polling
                executionPollInterval = setInterval(() => pollExecution(data.execution_id), 1000);
        ...
        async function pollExecution(executionId) {
            try {
                ...
                if (!res.ok) throw new Error('Failed to poll execution');
                ...
                // Add new logs
                if (data.logs) {
                    data.logs.forEach(log => addLog(log.level, log.message));
                }
        ...
        function updateProgress(percent, status) {
            ...
            document.getElementById('progress-label').textContent = status || 'Processing...';
        }

        function addLog(level, message) {
            const container = document.getElementById('execution-logs');
            const entry = document.createElement('div');
            entry.className = \`log-entry log-\${level} flex items-start gap-2\`;
            entry.innerHTML = \`
                <span class="text-gray-600">[\${new Date().toLocaleTimeString()}]</span>
                <span class="uppercase text-[10px] px-1 rounded bg-gray-800">\${level}</span>
                <span>\${message}</span>
        ...
        function showResults(data) {
            ...
            document.getElementById('result-icon').className = \`fas \${isSuccess ? 'fa-check-circle text-green-400' : 'fa-times-circle text-red-400'} text-xl\`;
            document.getElementById('result-status').textContent = isSuccess ? 'Completed' : 'Failed';
            document.getElementById('result-status').className = isSuccess ? 'font-medium text-green-400' : 'font-medium text-red-400';
            document.getElementById('result-time').textContent = data.duration ? \`Took \${data.duration}\` : '';
            document.getElementById('result-output').textContent = data.output || 'No output';
            
            // Evidence
            if (data.evidence && data.evidence.length > 0) {
                document.getElementById('result-evidence').classList.remove('hidden');
                document.getElementById('evidence-list').innerHTML = data.evidence.map(file => \`
                    <a href="\${file.url}" target="_blank" class="flex items-center gap-2 text-sm text-[#c17767] hover:underline">
            ...
            document.getElementById('execution-status').textContent = isSuccess ? 'Completed' : 'Failed';
            document.getElementById('status-dot').className = \`w-2 h-2 rounded-full \${isSuccess ? 'bg-green-400' : 'bg-red-400'}\`;
        ...
        function showError(message) {
            document.getElementById('execution-progress').classList.add('hidden');
            document.getElementById('execution-results').classList.remove('hidden');
            document.getElementById('result-icon').className = 'fas fa-times-circle text-red-400 text-xl';
            document.getElementById('result-status').textContent = 'Failed';
            document.getElementById('result-status').className = 'font-medium text-red-400';
        ...
        async function stopExecution() {
            ...
            try {
                await fetch(\`/api/executions/\${currentExecution}/stop\`, { method: 'POST' });
                clearInterval(executionPollInterval);
                addLog('info', 'Execution stopped by user');
                showResults({ status: 'cancelled', output: 'Execution cancelled by user' });
            } catch (err) {
        ...
        function resetExecution() {
            document.getElementById('execution-form').classList.remove('hidden');
            document.getElementById('execution-progress').classList.add('hidden');
            ...
            document.getElementById('execution-input').value = '';
            document.getElementById('execution-logs').innerHTML = '';
            document.getElementById('result-evidence').classList.add('hidden');
            currentExecution = null;
        }

        function createTask() {
            window.location.href = \`/?createTask=\${agentId}\`;
        }
